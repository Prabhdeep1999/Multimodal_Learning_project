...
for i in tqdm(range(len(files))):
    ...
    print(f'type feat: {type(feat)}')
    first_half = feat[:,:int(feat.shape[1]/2)]
    print(f'first_half.shape: {first_half.shape}')
    second_half = feat[:,int(feat.shape[1]/2):]
    print(f'second_half.shape: {second_half.shape}')
    averaged_video = (first_half + second_half)/2
    print(f'averaged_video.shape: {averaged_video.shape}')
    feat = averaged_video
